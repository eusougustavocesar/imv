...
export function UsersTable({ users }: { users: UserRow[] }) {
  if (users.length === 0) {
    return <p className="text-body-sm text-imv-subtle text-center py-6">Nenhum usuário.</p>
  }
        ...
        <thead>
          <tr className="bg-imv-dark">
            {["Usuário", "Role", "Desde"].map(h => (
              <th key={h} className="text-left px-4 py-3 text-meta font-semibold uppercase tracking-[0.08em] text-white/60 first:rounded-tl-xl last:rounded-tr-xl">
                {h}
              </th>
            ))}
          </tr>
  ...
  return (
    <tr className="bg-white hover:bg-imv-cream transition-colors">
      <td className="px-4 py-3">
        ...
        <p className="text-meta text-imv-subtle">{user.email}</p>
      </td>
      <td className="px-4 py-3">
        <button
          ...
          disabled={saving}
          className={[
            "text-meta font-bold uppercase tracking-[0.08em] px-3 py-1 rounded border transition-all disabled:opacity-50",
            role === "admin"
